...
class BFSPlanner(Node):
    def __init__(self):
        ...
        # Publishers
        self.path_pub = self.create_publisher(Path, '/bfs_path', 10)
        self.actual_trajectory_pub = self.create_publisher(Path, '/bfs_actual_trajectory', 10)
        self.nav2_plan_pub = self.create_publisher(Path, '/plan', 10)
        self.expansion_pub = self.create_publisher(MarkerArray, '/bfs_expansion', 10)
    ...
    def track_robot_position(self):
        ...
        if self.is_tracking_goal:
            # Append current robot position to live actual trajectory path
    ...
    def goal_callback(self, msg):
        ...
        # Reset actual trajectory path recording
        self.actual_trajectory_msg = Path()
        self.actual_trajectory_msg.header.frame_id = 'map'
        self.actual_distance_traveled = 0.0
    ...
    def execute_nav2_goal(self, path_msg):
        ...
        # Pass orientations to generated waypoints for smooth Nav2 rotation
        poses = path_msg.poses
    ...
    def plan_path(self):
        ...
        while queue:
            ...
            for dx, dy in moves:
                ...
                if 0 <= nx < width and 0 <= ny < height:
                    idx = ny * width + nx
                    if (grid[idx] == 0 or grid[idx] == -1) and (nx, ny) not in visited:
                        visited[(nx, ny)] = current
                        queue.append((nx, ny))
        
        comp_time = (time.perf_counter() - t_start_plan) * 1000.0
        self.publish_expansion_visualization(visited.keys())

        if not found:
            self.get_logger().error("BFS Search Failed: Path blocked or goal in obstacle!")
            self.is_tracking_goal = False
            return
        ...
        print("\n" + "="*55)
        print("           2. BFS PATH PLANNING METRICS")
        print("="*55)
        ...
        print(f" Explored Nodes Count  : {len(visited)} cells")
        print("="*55 + "\n")

        # Execute navigation via Nav2
