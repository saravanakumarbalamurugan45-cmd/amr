...
class AStarPlanner(Node):
    def __init__(self):
        ...
        self.actual_trajectory_pub = self.create_publisher(Path, '/astar_actual_trajectory', 10)
        self.nav2_plan_pub = self.create_publisher(Path, '/plan', 10)
        self.expansion_pub = self.create_publisher(MarkerArray, '/astar_expansion', 10)
    ...
    def plan_path(self):
        ...
        if not found:
            self.get_logger().error("A* Search Failed: Path blocked or goal in obstacle!")
            self.is_tracking_goal = False
            return
        ...
        print("\n" + "="*55)
        print("           2. A* PATH PLANNING METRICS")
        print("="*55)
